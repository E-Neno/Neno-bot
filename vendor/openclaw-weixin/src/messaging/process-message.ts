...
function summarizeInboundItemTypes(full: WeixinMessage): string {
  return full.item_list?.map((item) => item.type ?? "none").join(",") ?? "none";
}

function hasInboundImageItem(full: WeixinMessage): boolean {
  return Boolean(full.item_list?.some((item) => item.type === MessageItemType.IMAGE));
}
...
export async function processOneMessage(
  ...
  const finalizedWithBridgeMetadata = attachBridgeInboundMetadata(
    ...
    await deps.channelRuntime.reply.withReplyDispatcher({
      ...
      run: () =>
        deps.channelRuntime.reply.dispatchReplyFromConfig({
          ctx: finalizedWithBridgeMetadata,
          cfg: deps.config,
          dispatcher,
