...
class MessageProcessingDelegate(
    ...
    fun runExternalChatTurn(
        chatId: String,
        clearDraft: Boolean,
        block: suspend () -> List<ChatMessage>,
    ): Boolean {
        val chatRuntime = runtimeFor(chatId)
        if (chatRuntime.isLoading.value) return false

        if (clearDraft) clearUserMessageDraft(chatId)
        chatRuntime.responseStream = null
        ...
        chatRuntime.sendJob = coroutineScope.launch(Dispatchers.IO) {
            try {
                block().forEach { message -> addMessageToChat(chatId, message) }
                forceEmitScrollToBottom(chatId)
            } catch (cancelled: CancellationException) {
                throw cancelled
            } catch (throwable: Throwable) {
                AppLogger.e(TAG, "External chat turn failed: chatId=$chatId", throwable)
                showErrorMessage(throwable.message ?: "Neno request failed")
            } finally {
