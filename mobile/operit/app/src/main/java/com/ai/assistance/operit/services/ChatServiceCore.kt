...
import com.ai.assistance.operit.data.model.NenoRemoteSession
import com.ai.assistance.operit.data.remote.NenoRemoteConfig
import com.ai.assistance.operit.data.remote.NenoRemoteConfigStore
import com.ai.assistance.operit.data.remote.NenoRemoteTransport
import com.ai.assistance.operit.data.remote.isNenoRemoteChat
import com.ai.assistance.operit.data.remote.toChatMessage
import com.ai.assistance.operit.data.repository.NenoRemoteRepository
...
class ChatServiceCore(
    ...
    private val nenoRemoteConfigStore = NenoRemoteConfigStore(context)
    ...
    fun sendUserMessage(
        ...
        if (isNenoRemoteChat(targetChatId)) {
            sendNenoRemoteMessage(
                chatId = requireNotNull(targetChatId),
    ...
    fun refreshNenoRemoteHistory(chatId: String) {
        if (!isNenoRemoteChat(chatId)) return
        val config = nenoRemoteConfigStore.read()
        if (!config.isConfigured) {
            uiStateDelegate.showErrorMessage("Neno remote connection is not configured")
            return
        }
        val repository = createNenoRemoteRepository(config)
        messageProcessingDelegate.runExternalChatTurn(chatId, clearDraft = false) {
            val messages = repository.refresh(limit = 100)
            repository.draft.value.takeIf { it.isNotBlank() }?.let {
                messageProcessingDelegate.updateUserMessage(it)
            }
            messages.map { it.toChatMessage() }
        }
    }

    private fun sendNenoRemoteMessage(chatId: String, messageTextOverride: String?) {
        val text = (messageTextOverride ?: messageProcessingDelegate.userMessage.value.text).trim()
        val localAttachments = attachmentDelegate.attachments.value
        if (text.isBlank() && localAttachments.isEmpty()) return
        ...
        val repository = createNenoRemoteRepository(config)
        messageProcessingDelegate.runExternalChatTurn(
            chatId = chatId,
            clearDraft = true,
        ) {
            repository.saveDraft(text)
            val remoteAttachments = localAttachments.map { attachment ->
                val kind = when {
                    attachment.mimeType.startsWith("image/", ignoreCase = true) -> "image"
                    attachment.mimeType.startsWith("audio/", ignoreCase = true) -> "voice"
                    else -> "file"
                }
                val bytes = File(attachment.filePath).takeIf { it.isFile }?.readBytes()
                    ?: attachment.content.toByteArray(Charsets.UTF_8)
            ...
            val result = repository.send(text = text, attachments = remoteAttachments)
            attachmentDelegate.clearAttachments()
            listOfNotNull(result.userMessage, result.assistantMessage).map { it.toChatMessage() }
        }
    }

    private fun createNenoRemoteRepository(config: NenoRemoteConfig): NenoRemoteRepository {
        val client = OkHttpClient.Builder()
            .connectTimeout(20, TimeUnit.SECONDS)
            .readTimeout(180, TimeUnit.SECONDS)
            .writeTimeout(60, TimeUnit.SECONDS)
            .build()
        return NenoRemoteRepository(
            context = context,
            session = NenoRemoteSession(config.backendUrl, config.accountId),
            transport = NenoRemoteTransport(config.backendUrl, config.token, client),
        )
    }
