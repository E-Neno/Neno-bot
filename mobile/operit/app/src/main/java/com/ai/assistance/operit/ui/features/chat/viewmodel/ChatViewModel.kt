...
class ChatViewModel(private val context: Context) : ViewModel() {
    ...
    init {
        ...
        viewModelScope.launch {
            currentChatId
                ...
                .collect { chatId ->
                    if (chatId != null && com.ai.assistance.operit.data.remote.isNenoRemoteChat(chatId)) {
                        mainChatCore.refreshNenoRemoteHistory(chatId)
                    }
                }
        }
    ...
    fun switchActiveCharacterTarget(target: CharacterSelectorTarget) {
        viewModelScope.launch {
            when (target) {
                CharacterSelectorTarget.NenoRemoteTarget -> {
                    openNenoRemoteChat()
                    return@launch
                }
    ...
    private suspend fun autoSwitchChatForCharacterTarget(target: CharacterSelectorTarget) {
        runCatching {
            when (target) {
                CharacterSelectorTarget.NenoRemoteTarget -> openNenoRemoteChat()
                is CharacterSelectorTarget.CharacterCardTarget -> {
    ...
    private fun openNenoRemoteChat() {
        val config = nenoRemoteConfigStore.read()
        val chatId = NenoRemoteSession(config.backendUrl, config.accountId).chatKey
        chatHistoryDelegate.openNenoRemoteChat(chatId) { readyChatId ->
            chatRuntimeHolder.syncMainChatSelectionToFloating(readyChatId)
        }
    }
